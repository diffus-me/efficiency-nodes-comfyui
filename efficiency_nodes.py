...
class TSC_EfficientLoader:
    ...
    def efficientloader(self, ckpt_name, vae_name, clip_skip, lora_name, lora_model_strength, lora_clip_strength,
        ...
        if lora_name != "None" or lora_stack:
            ...
            # Load LoRA(s)
            model, clip = load_lora(context, lora_params, ckpt_name, my_unique_id, cache=lora_cache, ckpt_cache=ckpt_cache,
            ...
            if vae_name == "Baked VAE":
                ...
                if vae is None:
                    ...
                    _, _, vae = load_checkpoint(context, ckpt_name, my_unique_id, output_vae=True, cache=ckpt_cache,
                                                cache_overwrite=True)
        else:
...
class TSC_EfficientLoaderSDXL(TSC_EfficientLoader):
    ...
    def efficientloaderSDXL(self, base_ckpt_name, base_clip_skip, refiner_ckpt_name, refiner_clip_skip, positive_ascore,
                            ...
                            prompt=None, my_unique_id=None, context: execution_context.ExecutionContext = None):
        ...
        return super().efficientloader(base_ckpt_name, vae_name, clip_skip, lora_name, lora_model_strength, lora_clip_strength,
                        ...
                        ascore=(positive_ascore, negative_ascore), prompt=prompt, my_unique_id=my_unique_id, context=context, loader_type="sdxl")
...
class TSC_KSampler:
    ...
    def sample(self, model, seed, steps, cfg, sampler_name, scheduler, positive, negative, latent_image,
        ...
        def vae_decode_latent(vae, samples, vae_decode):
            return VAEDecodeTiled().decode(vae,samples,320, 32)[0] if "tiled" in vae_decode else VAEDecode().decode(vae,samples)[0]

        def vae_encode_image(vae, pixels, vae_decode):
            return VAEEncodeTiled().encode(vae,pixels,320, 32)[0] if "tiled" in vae_decode else VAEEncode().encode(vae,pixels)[0]
        ...
        def process_latent_image(model, seed, steps, cfg, sampler_name, scheduler, positive, negative, latent_image,
                                ...
                                refiner_model, refiner_positive, refiner_negative, vae, vae_decode, preview_method, user_hash):
            print(f'process_latent_image, user_hash: {user_hash}')
            # Store originals
        ...
        if not keys_exist_in_script("xyplot"):
            ...
            samples, images, gifs, preview = process_latent_image(model, seed, steps, cfg, sampler_name, scheduler,
                                            ...
                                            refiner_positive, refiner_negative, vae, vae_decode, preview_method, context.user_hash)
        ...
        preview_images = PreviewImage().save_images(xy_plot_image, user_hash=context.user_hash)["ui"]["images"]
...
class TSC_KSamplerAdvanced(TSC_KSampler):
    ...
    def sample_adv(self, model, add_noise, noise_seed, steps, cfg, sampler_name, scheduler, positive, negative,
               ...
               prompt=None, extra_pnginfo=None, my_unique_id=None, context: execution_context.ExecutionContext = None, optional_vae=(None,), script=None):
        ...
        return super().sample(model, noise_seed, steps, cfg, sampler_name, scheduler, positive, negative,
               ...
               optional_vae=optional_vae, script=script, add_noise=add_noise, start_at_step=start_at_step, context=context, end_at_step=end_at_step,
...
class TSC_KSamplerSDXL(TSC_KSampler):
    ...
    def sample_sdxl(self, sdxl_tuple, noise_seed, steps, cfg, sampler_name, scheduler, latent_image,
               ...
               my_unique_id=None, context: execution_context.ExecutionContext = None, optional_vae=(None,), refiner_extras=None, script=None):
        ...
        return super().sample(sdxl_tuple, noise_seed, steps, cfg, sampler_name, scheduler,
               ...
               prompt=prompt, extra_pnginfo=extra_pnginfo, my_unique_id=my_unique_id, context=context, optional_vae=optional_vae,
    ...
    def hires_fix_script(self, upscale_type, hires_ckpt_name, latent_upscaler, pixel_upscaler, upscale_by,
        ...
        if iterations > 0 and upscale_by > 0:
            if upscale_type == "latent":
                ...
                pixel_upscale_model = UpscaleModelLoader().load_model(pixel_upscaler, context)[0]
        ...
        control_net = ControlNetLoader().load_controlnet(control_net_name, context)[0] if use_controlnet is True else None
...
class TSC_Tiled_Upscaler:
    ...
    def tiled_sampling(self, upscale_by, tile_size, tiling_strategy, tiling_steps, seed, denoise,
        ...
        if tiling_strategy != 'none':
            script = script or {}
            tile_controlnet = ControlNetLoader().load_controlnet(tile_controlnet, context)[0] if use_controlnet else None
